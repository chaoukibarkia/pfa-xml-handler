...
class XMLParserService {
    ...
    setupReferenceListParsing(xml) {
        ...
        // In src/services/xml-parser-service.js, modify the NameType handler:
        xml.on('endElement: NameType', async (nameType) => {
            try {
                // Skip empty name types
                if (!nameType._ || nameType._.trim() === '') {
                    logger.processInfo('Skipped empty name type', {
    ...
    setupRecordParsing(xml) {
        ...
        xml.on('endElement: Person', async (person) => {
            try {
                ...
                this.incrementProcessedRecords();
                // Add these lines for batch memory management
                this.state.batchCounter++;
                if (this.state.batchCounter % this.config.batchSize === 0) {
                    // Force garbage collection if available
                    if (global.gc) {
                        global.gc();
                    }
                    logger.processInfo(`Processed ${this.state.batchCounter} records, memory cleanup triggered`, {
                        recordType: 'person',
                        batchNumber: Math.floor(this.state.batchCounter / this.config.batchSize)
                    });
                }
            } catch (error) {
